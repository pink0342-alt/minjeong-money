...
const defaults={weeklyBudget:100000,categories:["식비","카페·간식","교통","쇼핑","생활","기타"],pots:[],expenses:[],updatedAt:Date.now()};
...
function todaySpendable(){let rem=Math.max(0,data.weeklyBudget-weeklySpent()), now=new Date(), end=endOfWeek();let days=Math.max(1,Math.floor((end-new Date(now.getFullYear(),now.getMonth(),now.getDate()))/86400000)+1);return Math.floor(rem/days)}
...
document.addEventListener("click",e=>{let id=e.target.dataset.del;if(id){data.expenses=data.expenses.filter(x=>x.id!==id);save()}let dc=e.target.dataset.delcat;if(dc!==undefined&&data.categories.length>1){data.categories.splice(Number(dc),1);save()}let dp=e.target.dataset.delpot;if(dp){data.pots=data.pots.filter(x=>x.id!==dp);save()}let day=e.target.closest("[data-day]")?.dataset.day;if(day){selectedDay=day;renderCalendar()}});
...
function fillCloud(){let c=cloud();$("#sbUrl").value=c.url||"";$("#sbKey").value=c.key||"";$("#syncKey").value=c.syncKey||""} fillCloud();
...
async function syncData(silent=false){let c=cloud();if(!c.url||!c.key||!c.syncKey){if(!silent)alert("먼저 동기화 연결 정보를 저장해줘!");return}
 ...
 let merged=remote&&remote.updatedAt>data.updatedAt?remote:data; if(remote&&remote.updatedAt>data.updatedAt){data={...defaults,...remote};localStorage.setItem(KEY,JSON.stringify(data))}
